    ...
    try:
        with sqlite3.connect("chansons.db") as db:
            ...
            cursor.execute(
                ...
                (song.ISRC, song.titre, song.artiste, song.duree, song.img),
            )
            db.commit()
...
def MAJ_chanson(attribut, valeur, ISRC):
    ...
    try:
        ...
        print(
            "Une erreur est survenue lors de la mise à jour de la chanson d'IRSC {ISRC}"
        )
...
def supprimer_chanson(ISRC):
    ...
    try:
        ...
        print(
            "Une erreur est survenue lors de la suppression de la chanson d'IRSC {ISRC}"
        )
